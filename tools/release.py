...
def update_translations():
    url = "https://translate.codeberg.org/download/AnaBoard/?format=zip"
    zip_file_name = "translations.zip"
    urlretrieve(url, zip_file_name)
    # extract all in AnaBoard/AnaBoard/app/src/main/res and AnaBoard/AnaBoard/fastlane/metadata
    with zipfile.ZipFile(zip_file_name, "r") as f:
        for file in f.filelist:
            if not file.filename.startswith("AnaBoard/AnaBoard/app/src/main/res")\
                    and not file.filename.startswith("AnaBoard/AnaBoard/fastlane/metadata"):
                continue
            file.filename = file.filename.replace("AnaBoard/AnaBoard/", "")
            f.extract(file)
...
# for convenient linking when adding dictionaries in AnaBoard.
